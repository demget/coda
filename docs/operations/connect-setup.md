...
2. Allow both redirect URIs: `http://127.0.0.1:34338/callback` and
   `https://app.t3.codes/connect/callback`. A custom `CODA_HOSTED_APP_URL` needs its own
...
4. Set `CODA_CLERK_CLI_OAUTH_CLIENT_ID` to the generated public client ID in local and release
   build environments.
...
Set `CODA_CLERK_JWT_TEMPLATE=t3-relay` for clients and
