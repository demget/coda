...
Linked worktrees default to their own `.t3/userdata`, even when `CODA_HOME` is set.
...
but occupied ports can shift them. `CODA_PORT_OFFSET` or `CODA_DEV_INSTANCE` can select a
...
`CODA_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
