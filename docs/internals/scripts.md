...
- `vp run dev --share`: Also publishes the web port over HTTPS on this machine's tailnet. The
  ...
  Shared runs default to Vite's bundled dev mode (`CODA_BUNDLED_DEV=1`): a remote browser pays a
  network round trip per import level in unbundled dev, which turns a cold module graph into
  minutes of waterfall. Set `CODA_BUNDLED_DEV=0` to opt a shared run back out.
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `CODA_NO_BROWSER` itself from this flag, so setting `CODA_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/coda-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.coda`, even
  when `CODA_HOME` is set, storing state in `<worktree>/.coda/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.coda/dev`, keeping development state
  separate from `~/.coda/userdata`. An explicit `--home-dir <path>` stores state under
...
When `CODA_DESKTOP_REUSE_RESOURCE_MONITOR=true` points the build at an existing resource monitor,
...
- Desktop production windows load the bundled UI from the `coda://app/` root URL (not a
...
- Signed macOS builds also require `CODA_APPLE_TEAM_ID` and
  `CODA_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `CODA_CLERK_PUBLISHABLE_KEY` unless `CODA_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `CODA_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `CODA_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `CODA_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
