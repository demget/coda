...
```dotenv
CODA_CLERK_PUBLISHABLE_KEY=<publishable key>
CODA_CLERK_JWT_TEMPLATE=<JWT template name>
CODA_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
CODA_RELAY_URL=https://relay.example.com
```
...
should set `CODA_CLERK_PUBLISHABLE_KEY`, `CODA_CLERK_JWT_TEMPLATE`,
`CODA_CLERK_CLI_OAUTH_CLIENT_ID`, and `CODA_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `coda connect` command group is
...
The `coda connect` commands authorize a headless environment with a separate Clerk OAuth application.
...
5. Set `CODA_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
`coda connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `coda connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running T3 server. The next `coda serve` or `coda start` reconciles the relay link and launches the
managed tunnel. `coda connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `coda connect link` can re-enable exposure without another browser flow. `coda connect
...
logout leaves it running; manage it with `coda service status`, `install`, `update`, and `uninstall`.
...
Set `CODA_CLERK_JWT_TEMPLATE=t3-relay` in the repository-root `.env`, and set
...
is shared by production and non-production relay stages. The client-facing `CODA_RELAY_URL` still
...
Local desktop development uses `coda-dev://app`, while packaged builds use `coda://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `coda-dev://app`; the production Clerk instance should only need
`coda://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.coda.app`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.coda.app` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.coda.app`.
...
```dotenv
CODA_APPLE_TEAM_ID=ABC1234567
CODA_MACOS_PROVISIONING_PROFILE=/absolute/path/to/t3code.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
CODA_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `CODA_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`CODA_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `CODA_PORT` set. Rebuild the signed app
