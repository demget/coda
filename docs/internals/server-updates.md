...
The launcher is the only runtime writer of `service-state.json`. `coda service install` and
`coda service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `coda@<target>` into a unique staging directory.
...
```sh
npx coda@<version> service update
```
