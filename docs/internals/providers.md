...
Complete releases live in immutable version directories under the Coda home
...
scope. Only the initiating Coda auth session receives its URL and flow ID or can complete or
...
event drain, then sends the replacement. Native background commands use Coda's existing
...
`interaction_` requests are user questions, not approvals. Coda keeps opaque option IDs in
...
`showInteractionModeToggle=false` keeps native `/plan` separate from Coda Plan mode.
...
Coda does not own an external OpenCode process. Native configuration changes there can require
an external reload or restart before Coda's next refresh sees them.
