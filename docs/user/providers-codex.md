...
This guide is for people who want to use more than one Codex account in Coda. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Coda](./install.md).
...
In Coda Settings, name it something obvious:
...
In Coda Settings, add another Codex provider:
...
Coda shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Coda treats it
