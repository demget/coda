...
Connect a phone, browser, or another desktop app to Coda running on a different
...
```bash
npx coda serve --host <private-ip>
```
...
`--tailscale-serve-port`. See `npx coda pair --help` for other pairing options.
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. Coda starts
...
The first launch downloads the server to `~/.coda/runtime` on the host, so
...
the connection stops a server that Coda launched; a server that was already
...
management is available through `npx coda auth --help`.
...
On a command-line host, `coda connect unlink` disables exposure while retaining
your login; `coda connect logout` also clears that login. Background-service
...
Run `coda connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `coda service status` and read the displayed log. If it disappears
when SSH closes, see [background-service troubleshooting](./background-service.md#troubleshooting).

| Error                                                     | Recovery                                                                                                                                          |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart Coda on the host.                                                                                  |
| `auth_invalid` or `invalid_bearer`                        | Run `coda connect login`. If credentials were revoked, run `coda connect logout`, then `coda connect` again. Restart the server after signing in. |
| Expired or invalid link proof                             | Check the host's date and time, update Coda, then restart it.                                                                                     |
...
foreground server, stop it and run `coda serve` again with your usual options.
...
devices. For server version warnings, follow [Updating Coda](./updating.md).
...
environment**. Coda restarts without a local server: no local agents or terminals run, WSL
