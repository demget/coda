...
Coda runs coding agents on your computer and lets you control them from its
...
launch Coda and configure providers afterwards.
...
`npx coda@latest --help` for command-line options.
...
Pass a path, such as `npx coda app ../my-project`, to open another directory. It requires
the desktop app, so a standalone server or an SSH session is not enough. If the
command cannot reach the app, start or update the desktop app and try again.

## Mobile app

Install Coda from the
...
| Antigravity | Install and sign in with Google from Coda's provider settings.                               |

Provider CLIs must be on the server's `PATH`. If Coda cannot find one, set its
...
base URL. Mark secret values as sensitive; after saving, Coda does not display
...
- [Updating Coda](./updating.md): update the app and connected servers.
