...
Messages can contain up to 120,000 characters. If a draft is longer, Coda keeps it in the
...
Coda from other apps through the system share sheet. Mobile keeps a local copy of each draft
...
Coda remembers the last provider, model, and model options you selected and reuses that
...
Model options shown as provider defaults remain display values until you choose them in Coda.
Coda only sends options you selected explicitly, so an unset reasoning level or service tier can
...
readable and Coda shows a warning.
...
On web, desktop, and mobile, select a link to an image or video to open it inside Coda.
Workspace image and video links open the file viewer. Links to media outside the workspace
open a media preview.
Videos opened from the file explorer or a file-viewer tab also play inside Coda. They
...
Coda serves the original file without adding it to attachment storage. If that file is moved
...
runs in an isolated frame with no access to your Coda session. On desktop, the integrated
...
project exists there. If it does not, Coda selects another environment that has the project.
...
and attachments. Coda deletes the local audio file after transcription or cancellation. It sends
...
also reports that skill as a native slash command, Coda hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

A skill token runs the skill wherever it sits in your message. Coda sends it to each provider in
...
them only there. Coda's own commands, such as `/model` and `/plan`, and skills stay available on
any line.

On desktop, press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux from a new thread to
start it in the background. Coda opens another new thread and shows an **Open** action for the
