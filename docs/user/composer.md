...
also send files to Coda through another app's system share sheet.
...
Coda remembers your provider, model, and model options for new threads. A
...
Transcription runs on your device. Coda deletes the temporary audio after
...
Provider commands must start the message to run. Coda commands such as
...
automatically. HTML previews cannot access your Coda session.
