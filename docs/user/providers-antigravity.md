...
Coda runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Coda to confirm
...
hostname. Only that Coda sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Coda's separate Plan mode
...
Coda keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Coda also reads `.gemini/skills` and the
