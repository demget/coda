...
Coda runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this Coda provider.
...
2. Choose **Install Antigravity**. Coda downloads the official runtime from Google to that
...
6. Wait for Coda to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the Coda environment, sign-in can finish directly.
...
return URL field in Coda. Choose **Continue** on web or desktop, or **Complete sign-in** on
mobile. Do not change the address to your server's hostname.

Return to the same Coda client and environment where you started sign-in. Another client
...
a thread or bug report. Coda waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in Coda settings on that environment and are passed only to the
...
Coda does not download it until you choose to install it.

On web or desktop, **Update Antigravity** appears when Coda has a newer managed release.
...
A nonempty **Binary path** takes priority. With the field empty, Coda uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. Coda does not update or remove manual
installations. Clear **Binary path** to use managed installation controls.

## Models and threads

The current official ACP exposes Gemini models only. Coda uses the model IDs and names
...
Coda asks you to select an available model instead of silently changing it.

Use Antigravity's native `/plan` command to request a plan. Coda's separate Plan mode control
is not available for this provider.

Antigravity reads and edits workspace files through Coda. Each write shows up as a file
...
When the agent offers **Allow for this thread** on a shell or web tool, Coda shows Google's
...
Coda keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
installation. Coda refuses removal while the runtime is in use. An explicit **Binary path**
that points into the managed runtime must be cleared first.

## Account access and errors

Google controls account eligibility, models, and usage limits. Coda does not report your
...
Use any retry time Google supplies. Coda does not switch to an API key to get past the limit.
