...
machine running your Coda server does that, its themes appear in the theme library alongside
your own, and Coda follows them: change the desktop theme and Coda retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. Coda Mobile keeps its own
...
the standard Coda look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Coda state
...
exported from Coda (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Coda generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Coda theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Coda does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Coda never reads
