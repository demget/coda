...
On Linux and macOS, Coda can run as a service for your user so you do not need
to keep a terminal open.

## Manage the service

Run these commands on the machine that will host Coda:

| Task                            | Command                             |
| ------------------------------- | ----------------------------------- |
| Install and start               | `npx coda@latest service install`   |
| Inspect status and log location | `npx coda@latest service status`    |
| Update or repair                | `npx coda@latest service update`    |
| Stop and remove from startup    | `npx coda@latest service uninstall` |

Uninstalling the service leaves your projects, threads, and settings intact.

Install and update use the version of the CLI you invoke. For nightly, use
`npx coda@nightly service update`; replace `nightly` with an exact version to pin
...
[Updating Coda](./updating.md).

## Platform support

Linux needs systemd user services. Setup enables lingering so Coda starts at
...
Start with `coda service status` on the host. It prints the log path and, on Linux,
...
with sudo; running Coda as root creates a separate installation and Connect
identity. Without administrator access, run `coda serve` in a terminal and keep
that session open.
...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status t3code.service`, then use the repair command printed by Coda.                        |
