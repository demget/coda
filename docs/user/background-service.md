...
On Linux and macOS, Coda can run as a background service for your user, so it is ready without
...
The service uses the same Coda version as the CLI you run. To install a nightly or an exact
...
The service runs a small stable launcher. Exact Coda versions are installed separately, so a
...
**Linux** uses a systemd user unit at `~/.config/systemd/user/coda.service`. The service starts
when the machine boots and keeps running after you log out (lingering is enabled during install).

**macOS** uses a launch agent at `~/Library/LaunchAgents/com.coda.app.service.plist`. It
...
Signing out of T3 Connect does not remove the service. Use `coda service uninstall` when you no longer
