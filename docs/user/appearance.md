...
On web and desktop, choose **Create theme** to adjust a palette, or import a Coda or VS Code
...
takes precedence. If the server stops publishing the selected theme, Coda falls back to its
standard theme.
...
choosing another theme afterward sticks until the next `coda theme set`. Run the command again to
reapply it, even if the name is unchanged.

`coda theme clear` removes the default without changing anyone's current theme. `coda theme show` lists
the default and published themes.

### Publish a theme

Save a theme exported from Coda into `~/.t3/userdata/themes/` on the server, or the `themes`
directory under your custom state directory. The filename supplies the theme ID: `nightfall.json`
can be selected with `coda theme set nightfall`. Keep the filename stable when updating its colors.
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. Coda
