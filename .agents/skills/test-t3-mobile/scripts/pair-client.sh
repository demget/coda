...
if ! pairing_output="$({
  CODA_PORT="$server_port" node apps/server/src/bin.ts auth pairing create \
    --base-dir "$base_dir" \
...
process.stdout.write(`coda-dev://connections/new?${query}`);
NODE
)"

if ! "$agent_device_command" open com.coda.app.dev "$deep_link" "$@" \
  >/dev/null 2>&1; then
