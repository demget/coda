...
- App: `Coda Dev`
- Bundle/package identifier: `com.coda.app.dev`
- URL scheme: `coda-dev`
...
Run backend commands from the repository root. Use the ignored, worktree-local `.coda` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.coda` state.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `coda-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/CodaDev.xcworkspace`
- Scheme: `CodaDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.coda.app.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.coda.app.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.coda.app.dev
...
In PowerShell, set `$env:CODA_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'coda-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'coda-dev://connections/new' \
  com.coda.app.dev
```

Run only the command for the selected platform.

In Coda Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
