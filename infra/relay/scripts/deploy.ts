...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    CODA_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    ...
    CODA_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("CODA_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("CODA_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "CodaRelay", stage });
