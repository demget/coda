...
describe("PublishClientConfig", () => {
  test.provider("writes the env file on deploy and skips an unchanged redeploy", (stack) =>
    Effect.gen(function* () {
      ...
      expect(first).toContain("CODA_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-v1\n");
      expect(first).toContain("CODA_MOBILE_OTLP_TRACES_TOKEN=mobile-v1\n");
        ...
        .pipe(
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromUnknown({ CODA_RELAY_CLIENT_CONFIG_ENV: target }),
            ),
          ),
          Effect.exit,
        );
