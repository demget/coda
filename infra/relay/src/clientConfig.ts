...
export const relayClientConfigEnv = (config: RelayClientConfig & { readonly url: string }) =>
  ({
    CODA_RELAY_URL: config.url,
    CODA_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    CODA_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    CODA_MOBILE_OTLP_TRACES_TOKEN: Redacted.value(config.mobileTracingToken),
    CODA_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    CODA_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    CODA_RELAY_CLIENT_OTLP_TRACES_TOKEN: Redacted.value(config.clientTracingToken),
  }) as const;
 ...
 * deploy. Set `CODA_RELAY_CLIENT_CONFIG_ENV` to write elsewhere (CI does).
 */
...
export const PublishClientConfig = Alchemy.Action(
  ...
  Effect.gen(function* () {
    ...
    const override = yield* Config.String("CODA_RELAY_CLIENT_CONFIG_ENV").pipe(Config.option);
