...
describe("electron development launcher", () => {
  it("uses captured values only as fallbacks for a live runner environment", () => {
    ...
    assert.include(
      environmentScript,
      "if [ -z \"${CODA_OTLP_PROTOCOL:-}\" ]; then export CODA_OTLP_PROTOCOL='http/protobuf'; fi",
    );
  ...
  it("keeps the launcher script free of volatile environment values", () => {
    ...
    assert.include(
      script,
      "exec '/repo/node_modules/electron/Electron' --coda-dev-root='/repo/apps/desktop' '/repo/apps/desktop/dist-electron/main.cjs' \"$@\"",
    );
  });
  ...
  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("Coda (Dev) Launcher");
    ...
    assert.equal(
      ...
      "Coda reads project files you open in the desktop app.",
    );
  });

  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/Coda (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/Coda (Dev).app",
    ]);
  });
