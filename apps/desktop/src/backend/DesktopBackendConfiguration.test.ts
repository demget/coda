    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          CODA_PORT: "9999",
          CODA_MODE: "desktop",
          CODA_DESKTOP_LAN_HOST: "192.168.1.50",
          VITE_DEV_SERVER_URL: options?.devServerUrl,
          CODA_OTLP_TRACES_URL: options?.otlpTracesUrl,
          CODA_OTLP_METRICS_URL: options?.otlpMetricsUrl,
          CODA_OTLP_LOGS_URL: options?.otlpLogsUrl,
        }),
      ),
    ),
  );
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.CODA_PORT);
        assert.isUndefined(first.env.CODA_MODE);
        assert.isUndefined(first.env.CODA_DESKTOP_LAN_HOST);
    ...
    () =>
      Effect.gen(function* () {
        ...
        const standard = {
          ...
          CODA_OTLP_TRACES_URL: "http://t3.example.com:4318/v1/traces",
        };
        const previousWslEnv = process.env.WSLENV;
        // A developer's own OTLP variables would be forwarded too.
        const ambientOtel = Object.entries(process.env).filter(
          ([name]) => name.startsWith("OTEL_") || name.startsWith("CODA_OTLP_"),
        );
        try {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.equal(config.env.CODA_OTLP_TRACES_URL, "http://t3.example.com:4318/v1/traces");
            assert.include(wslEnv, "CODA_OTLP_TRACES_URL");
          }).pipe(
            Effect.provide(
              DesktopBackendConfiguration.layer.pipe(
                Layer.provideMerge(serverExposureLayer),
  ...
  it.effect("resolveWsl preserves existing WSLENV entries when forwarding backend secrets", () =>
    Effect.gen(function* () {
      ...
      const previousOtlpHeaders = process.env.CODA_OTLP_HEADERS;
      const previousOtlpProtocol = process.env.CODA_OTLP_PROTOCOL;
      ...
      try {
        ...
        process.env.CODA_OTLP_HEADERS = 'authorization="Bearer%20my-token"';
        process.env.CODA_OTLP_PROTOCOL = "http/protobuf";
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.env.CODA_OTLP_PROTOCOL, "http/protobuf");
          ...
          assert.equal(
            config.env.WSLENV,
            "GOPATH/p:OPENAI_API_KEY/u:EMPTY::AZURE_DEVOPS_EXT_PAT/u:ANTHROPIC_API_KEY:CODA_OTLP_HEADERS:CODA_OTLP_PROTOCOL",
          );
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
        ...
        restoreEnv("CODA_OTLP_HEADERS", previousOtlpHeaders);
        restoreEnv("CODA_OTLP_PROTOCOL", previousOtlpProtocol);
        for (const [name, value] of ambientOtel) restoreEnv(name, value);
      }
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
  );
