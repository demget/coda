...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "CODA_HOST",
  "CODA_DESKTOP_WS_URL",
  "CODA_DESKTOP_LAN_ACCESS",
  "CODA_DESKTOP_LAN_HOST",
  "CODA_DESKTOP_HTTPS_ENDPOINTS",
  "CODA_TAILSCALE_SERVE",
  "CODA_TAILSCALE_SERVE_PORT",
] as const;
...
const WSL_FORWARDED_ENV_NAMES = [
  ...
  // server prefers an OTEL endpoint over the bootstrap envelope, so the Coda URLs
  // travel as variables to keep winning inside the distro as they do on Windows.
  "CODA_OTLP_TRACES_URL",
  "CODA_OTLP_METRICS_URL",
  "CODA_OTLP_LOGS_URL",
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's CODA_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  // Build an explicit copy of process.env minus CODA_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.coda via
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus CODA_HOME; pass it
