...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ CODA_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { CODA_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("preserves explicit Tailscale exposure overrides", () =>
    withHarness(
      ...
      {
        CODA_DESKTOP_LAN_HOST: "100.90.1.2",
        CODA_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
