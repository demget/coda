...
export function makeHarness(options: UpdatesHarnessOptions = {}) {
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          CODA_HOME: `/tmp/coda-desktop-updates-test-${process.pid}`,
          CODA_DESKTOP_MOCK_UPDATES: "true",
          CODA_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
  ...
  const layer = DesktopUpdates.layer.pipe(
    Layer.provide(options.resourcesPath === undefined ? fileSystemLayer : NodeServices.layer),
