...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { CODA_HOME: "/home/user/.coda-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.coda-test/userdata/desktop-settings.json");
  ...
  it("keeps implicit development state under ~/.coda/dev when CODA_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.coda/dev/desktop-settings.json");
  ...
  it("treats whitespace-only CODA_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        CODA_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
