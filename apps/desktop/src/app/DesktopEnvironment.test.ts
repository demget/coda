...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        ...
        {
          CODA_HOME: " /tmp/t3 ",
          CODA_COMMIT_HASH: " 0123456789abcdef ",
          CODA_PORT: "4949",
      ...
      assert.equal(environment.linuxDesktopEntryName, "com.coda.app.Development.desktop");
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "com.coda.app.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.coda/dev");
      assert.equal(production.stateDir, "/Users/alice/.coda/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          CODA_DESKTOP_APP_USER_MODEL_ID: " com.coda.app.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.coda.app.dev.local");
    }),
  );
