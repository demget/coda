...
describe("coda pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        ...
        assert.equal(credentials[0]?.label, "coda pair");
      }),
    ).pipe(
  ...
  it.effect("directs to coda serve or coda connect when no server is running", () =>
    Effect.gen(function* () {
      ...
      assert.include(rendered, "npx coda serve");
      assert.include(rendered, "npx coda connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
