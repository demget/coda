...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("lets CODA_OTEL_SDK_DISABLED=false override an ambient OTEL_SDK_DISABLED=true", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  CODA_OTLP_HEADERS: "authorization=Bearer abc==, x-tenant=t3",
                  CODA_OTLP_TRACES_URL: "http://collector.internal:4318",
                },
              }),
            ),
            NetService.layer,
    ...
    Effect.gen(function* () {
      ...
      const resolved = yield* resolveServerConfig(
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({ env: { CODA_OTLP_PROTOCOL: "http/protobuf" } }),
            ),
            NetService.layer,
          ),
        ),
      ...
      const resolved = yield* resolveServerConfig(
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { CODA_OTLP_LOGS_URL: "http://collector.internal:4318/v1/logs" },
              }),
            ),
            NetService.layer,
          ),
  ...
  it.effect(
    "resolves each signal's endpoint through CODA_OTLP_*_URL, an OTEL endpoint, the bootstrap envelope, and persisted Settings, in that order",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
          ...
          Effect.provide(
            Layer.mergeAll(
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    CODA_OTLP_TRACES_URL: "http://t3:4318/v1/traces",
                    CODA_OTLP_HEADERS: "x-key=secret",
        ...
        // CODA_OTLP_TRACES_URL wins over the OTEL variable for the same
        ...
        // Metrics named no CODA_OTLP_METRICS_URL, so the OTEL endpoint wins
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // CODA_OTLP_TRACES_URL still wins outright.
        expect(resolved.otlpTracesUrl).toBe("http://t3:4318/v1/traces");
