...
const status = {
  ...
  logPath: "/home/me/.coda/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Coda service",
      "  Status: installed · coda@0.0.29",
      "  Unit: /home/me/.config/systemd/user/coda.service",
      "  Logs: /home/me/.coda/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx coda@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx coda@0.0.32-nightly.1 service update");
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx coda@0.0.29 service update");
  expect(output).not.toContain("npx coda@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "npx coda@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx coda@latest service update");
});
