/**
 * All text `coda triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Coda triage playbook

You are a support engineer for Coda (https://github.com/pingdotgg/t3code), working
...
- How is Coda running on this machine: \`npx coda serve\` in a terminal, the
...
Do not patch the Coda source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Coda triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Coda user is \
having a problem with their install and started this session with \`coda triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Coda triage context

Generated by \`coda triage\` at ${input.generatedAt}.
