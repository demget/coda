...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      "Coda service",
      `  Status: installed · coda@${installedVersion} (newer than this coda@${cliVersion} CLI)`,
      `  Unit: ${status.unitPath}`,
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Use \`npx coda@${installedVersion} service update\` to repair it, or pass \`--allow-downgrade\` explicitly.`,
    ].join("\n");
  }
  return [
    "Coda service",
    `  Status: ${status.current ? `installed · coda@${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : [`  Next: Run \`npx coda@${cliVersion} service update\`.`]),
  ].join("\n");
}
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription(
    "Update or repair the background service using this CLI version. Use `npx coda@latest service update` for the latest release.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (
    ...
    yield* Console.log(
      `A newer coda@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ...
        : platform === "darwin"
          ? "Run Coda in the background whenever you log in to this Mac? " +
