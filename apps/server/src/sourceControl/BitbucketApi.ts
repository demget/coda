...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("CODA_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("CODA_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("CODA_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("CODA_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("coda/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `coda/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set CODA_BITBUCKET_EMAIL and CODA_BITBUCKET_API_TOKEN, or CODA_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
