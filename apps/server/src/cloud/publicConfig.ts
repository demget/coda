...
declare const __CODA_BUILD_RELAY_URL__: string | undefined;
declare const __CODA_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __CODA_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __CODA_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.CODA_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset = env.CODA_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.CODA_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("CODA_RELAY_URL");
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.CODA_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.CODA_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.CODA_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
