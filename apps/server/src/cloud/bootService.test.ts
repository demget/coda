...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.coda/runtime/service-launcher.mjs",
  ...
  expect(unit).toContain("ExecStart=/usr/bin/node /home/theo/.coda/runtime/service-launcher.mjs");
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.coda.app.service.plist",
};
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now coda.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop coda.service", "systemctl --user restart coda.service"]
            : [
                "launchctl bootout --wait gui/501/com.coda.app.service",
  ...
  it.effect("restarts without overwriting a pending remote update", () =>
    Effect.gen(function* () {
      ...
      for (const allowDowngrade of [false, true]) {
        ...
        ).toEqual(["systemctl --user stop coda.service", "systemctl --user restart coda.service"]);
      }
    }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(path.join("Library", "LaunchAgents", "com.coda.app.service.plist")),
      ).toBe(true);
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.coda.app.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.coda.app.service",
        "launchctl enable gui/501/com.coda.app.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.coda.app.service";
