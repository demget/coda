...
// (com.coda.app), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "com.coda.app.service";
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "linger-disabled":
      return 'Lingering is disabled. Coda will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `coda service update` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status t3code.service`, then run `coda service update`.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedErrorClass<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace coda@${this.installedVersion} with older coda@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
