...
const resolveRealPathWithMissingTail = Effect.fn(
  "AntigravityAdapter.resolveRealPathWithMissingTail",
  ...
  for (;;) {
    ...
    if (Option.isSome(real)) return path.join(real.value, ...tail.toReversed());
...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    const parent = yield* resolveRealPathWithMissingTail(
      input.fileSystem,
      path,
