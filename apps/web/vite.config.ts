...
const isSingleOriginDev = process.env.CODA_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_CODA_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.CODA_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: CODA_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); CODA_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.CODA_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // CODA_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.CODA_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.CODA_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_CODA_RELAY_URL": JSON.stringify(configuredRelayUrl),
