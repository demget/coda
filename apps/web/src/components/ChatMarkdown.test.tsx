...
import { filePathExistenceStore } from "../lib/filePathExistence";
...
describe("ChatMarkdown Windows file links", () => {
  ...
  // The chip only renders once the environment confirms the file exists, so
  // seed the shared existence store with answers for every fixture path.
  beforeEach(async () => {
    filePathExistenceStore.reset();
    filePathExistenceStore.observe({
      environmentId,
      cwd: "C:/Users/shawn/project",
      paths: [
        "C:/Users/shawn/project/src/main.ts",
        "C:/Users/shawn/project/src/index.ts",
        "C:/Users/shawn/project/test/index.ts",
        String.raw`C:\Users\shawn\project\src\main.ts`,
      ],
      check: (target) => Promise.resolve(target.paths.map((path) => ({ path, exists: true }))),
    });
