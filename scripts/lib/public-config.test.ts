...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.CODA_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.CODA_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.CODA_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.CODA_RELAY_URL).toBeUndefined();
    expect(env.VITE_CODA_RELAY_URL).toBeUndefined();
    expect(env.CODA_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.CODA_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.CODA_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.CODA_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.CODA_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.CODA_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "CODA_CLERK_PUBLISHABLE_KEY=pk_root\nCODA_CLERK_JWT_TEMPLATE=template_root\nCODA_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nCODA_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "CODA_CLERK_PUBLISHABLE_KEY=pk_local\nCODA_CLERK_JWT_TEMPLATE=template_local\nCODA_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nCODA_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).CODA_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          CODA_CLERK_PUBLISHABLE_KEY: "pk_ci",
          CODA_CLERK_JWT_TEMPLATE: "template_ci",
          CODA_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          CODA_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      CODA_RELAY_URL: "https://ci.example.test",
      VITE_CODA_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        CODA_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_CODA_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          CODA_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          CODA_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          CODA_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          CODA_RELAY_URL: "https://relay.example.test",
          CODA_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          CODA_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          CODA_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      CODA_RELAY_URL: "https://relay.example.test",
      VITE_CODA_RELAY_URL: "https://relay.example.test",
      CODA_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      CODA_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      CODA_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
