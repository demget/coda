...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                CODA_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/t3code",
              },
            }),
          ),
        ),
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Coda (Alpha) 1.2.3 Installer",
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Coda", schemes: ["coda", "coda-dev"] },
      ]);
      ...
      Effect.gen(function* () {
        ...
        yield* stageResourceMonitor({
          ...
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { CODA_DESKTOP_REUSE_RESOURCE_MONITOR: "true" },
              }),
            ),
          ),
        );
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    ...
                    CODA_DESKTOP_REUSE_RESOURCE_MONITOR: "true",
                  },
                }),
              ),
            ),
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      CODA_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      CODA_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.coda.app</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      CODA_APPLE_TEAM_ID: "ABC1234567",
      CODA_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "CODA_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      CODA_APPLE_TEAM_ID: "ABC1234567",
      CODA_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      CODA_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          CODA_APPLE_TEAM_ID: "ABC1234567",
          CODA_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
          CODA_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      CODA_APPLE_TEAM_ID: "ABC1234567",
      CODA_MACOS_PROVISIONING_PROFILE: "/tmp/t3code.provisionprofile",
      CODA_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "CODA_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.protocols, [{ name: "Coda", schemes: ["coda", "coda-dev"] }]);
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                CODA_DESKTOP_SKIP_BUILD: "true",
                CODA_DESKTOP_KEEP_STAGE: "true",
                CODA_DESKTOP_SIGNED: "true",
                CODA_DESKTOP_VERBOSE: "true",
                CODA_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
