...
const DESKTOP_APP_ID = "com.coda.app";
...
export class InvalidAppleTeamIdError extends Schema.TaggedError<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `CODA_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedError<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "CODA_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "CODA_CLERK_PUBLISHABLE_KEY or CODA_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "CODA_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "CODA_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.String("CODA_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "CODA_DESKTOP_ARCH").pipe(Config.option),
  version: Config.String("CODA_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.String("CODA_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  ...
  // WSL runtime.
  wslRuntime: Config.String("CODA_DESKTOP_WSL_RUNTIME").pipe(Config.option),
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.Boolean("CODA_DESKTOP_REUSE_RESOURCE_MONITOR").pipe(
    Config.withDefault(false),
  );
  const reuseCaptureHelpers = yield* Config.Boolean(
    "CODA_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const reuseHelpers = yield* Config.Boolean("CODA_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS").pipe(
    Config.withDefault(false),
  );
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.String("CODA_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  arch?: typeof BuildArch.Type,
  // macOS only: ad-hoc sign unsigned builds with a stable designated
  // requirement so Squirrel.Mac accepts them as updates.
  macUpdaterAdHoc = false,
) {
  const buildConfig: Record<string, unknown> = {
    appId: DESKTOP_APP_ID,
    productName: resolveDesktopProductName(version),
    artifactName: "Coda-${version}-${arch}.${ext}",
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        ...
        NSScreenCaptureUsageDescription:
          "Coda captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "Coda",
          schemes: ["coda", "coda-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      ...
      // coda:// OAuth callbacks to the app.
      protocols: [
        {
          name: "Coda",
          schemes: ["coda", "coda-dev"],
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    // Required by the .deb control file.
    homepage: "https://github.com/demget/coda",
    author: "T3 Tools",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.Literals("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: CODA_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.String("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: CODA_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.Literals("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: CODA_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.String("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: CODA_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.String("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: CODA_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.Boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: CODA_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.Boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: CODA_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  ...
  verbose: Flag.Boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: CODA_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.Boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: CODA_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.Int("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: CODA_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslRuntime: Flag.String("wsl-runtime").pipe(
    Flag.withDescription(
      "Path to the Linux CLI release archive (t3-<version>-linux-x64.tar.gz) to embed as the WSL runtime of a Windows build (env: CODA_DESKTOP_WSL_RUNTIME).",
    ),
    Flag.optional,
  ),
}).pipe(
