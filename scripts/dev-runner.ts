...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "--filter=coda", "dev"],
...
/**
 * `role` matters because only the backend honours `--host`/`CODA_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("CODA_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("CODA_PORT_OFFSET"),
  devInstance: optionalStringConfig("CODA_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `CODA_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric CODA_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed CODA_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .coda > ambient CODA_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      output.CODA_HOME = resolvedBaseDir;
    } else {
      delete output.CODA_HOME;
    }
    ...
    if (!isDesktopMode) {
      output.CODA_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.CODA_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.CODA_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.CODA_PORT = String(serverPort);
      ...
      delete output.CODA_SINGLE_ORIGIN_DEV;
      delete output.CODA_MODE;
      delete output.CODA_NO_BROWSER;
      delete output.CODA_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.CODA_HOST = host;
    }

    if (!isDesktopMode) {
      output.CODA_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.CODA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.CODA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.CODA_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.CODA_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.CODA_MODE = "web";
      delete output.CODA_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`CODA_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["CODA_PORT_OFFSET", "CODA_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.coda` — see @t3tools/shared/devHome for why this must
    // outrank an ambient CODA_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.CODA_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.CODA_HOME ?? (yield* DEFAULT_T3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.CODA_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.CODA_DEV_ALLOWED_ORIGINS = [env.CODA_DEV_ALLOWED_ORIGINS, new URL(shared.url).origin]
          ...
          // a few chunk requests. Only defaulted, so CODA_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.CODA_BUNDLED_DEV === undefined) {
            env.CODA_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Coda data directory; runtime state is stored under userdata (equivalent to CODA_HOME). Inside a git worktree this defaults to that worktree's own .coda so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to CODA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("CODA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to CODA_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("CODA_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to CODA_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("CODA_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to CODA_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("CODA_PORT")),
  ),
