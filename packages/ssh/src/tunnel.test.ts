...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(script, "exec npx --yes 'coda@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'coda@latest' -- \"$@\"");
    assert.include(script, "could not install 'coda@latest'");
    assert.include(script, "require_installed_t3_cli npx --yes --package 'coda@latest'");
    assert.include(script, "require_installed_t3_cli npm exec --yes --package 'coda@latest'");
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    const script = buildRemoteT3RunnerScript({
      packageSpec: "coda@nightly; touch /tmp/t3-owned",
    });

    assert.include(script, "exec npx --yes 'coda@nightly; touch /tmp/t3-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'coda@nightly; touch /tmp/t3-owned' -- \"$@\"");
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --package 'coda@nightly; touch /tmp/t3-owned'",
    );
    assert.notInclude(script, "exec npx --yes coda@nightly; touch /tmp/t3-owned");
  });
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(buildRemoteLaunchScript({ packageSpec: "coda@nightly" }), "coda@nightly");
    assert.include(
