...
if ! t3_runtime_ready; then
  ...
  # under it.
  T3_LOCK="$HOME/.coda/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_STAGING="$(mktemp -d "$HOME/.coda/runtime/versions/.staging-XXXXXX")"
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.coda/ssh-launch/$STATE_KEY"
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env CODA_NO_BROWSER=1 "$RUNNER_FILE" serve --no-browser --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
