 ...
 *
 * `CODA_OTEL_SDK_DISABLED` is read first, so a machine that sets
...
const CODA_TRUE = ["true", "yes", "on", "1", "y"];
const CODA_FALSE = ["false", "no", "off", "0", "n"];
...
export const load: Effect.Effect<OtelEnvironment> = Config.all({
  t3: flag(
    "CODA_OTEL_SDK_DISABLED",
    CODA_TRUE,
    CODA_FALSE,
    (value) => `CODA_OTEL_SDK_DISABLED=${value} is not a yes or a no and was ignored`,
  ),
  ...
  Effect.map(({ t3, spec, resource, generic, ...own }) => {
    ...
    if (disabled) {
      warnings.push(
        t3.value
          ? "CODA_OTEL_SDK_DISABLED is set, so no telemetry is exported, whatever configured it"
...
/**
 * Where one signal exports and how. `CODA_OTLP_*_URL` wins outright with
 * T3 Code's own export, then an OTEL endpoint with its own headers and
 * protocol, since `CODA_OTLP_HEADERS` was written for a different
